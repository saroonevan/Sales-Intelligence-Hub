            ...
            if not st.session_state[guard_key]:
                if st.button("💳  Confirm Payment", use_container_width=True):
                    ...
                    run_write([
                        ...
                        ("UPDATE customer_sales SET received_amount=received_amount+%s WHERE sale_id=%s",
                         (pay_amount, sale["sale_id"])),
                ...
                if not confirm_rm:
                    ...
                    run_write([
                        ("DELETE FROM payment_splits WHERE payment_id=%s",
                         (pay_rm["payment_id"],)),
                        ("UPDATE customer_sales SET received_amount=received_amount-%s, status='Open' WHERE sale_id=%s",
                         (pay_rm["amount_paid"], sale["sale_id"])),
                    ])
